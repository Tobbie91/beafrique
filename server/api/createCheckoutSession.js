...
// api/createCheckoutSession.js (or .ts)
const Stripe = require("stripe");
let stripe = null;
function getStripe() {
  const key = process.env.STRIPE_SECRET_KEY || "";
...
// allowlist the domains you trust
const ALLOWED_ORIGINS = [
  process.env.SITE_URL,                     // e.g. https://beafrique.netlify.app
  "https://beafrique.netlify.app",
].filter(Boolean);

function pickReturnUrl(raw) {
  try {
    const u = new URL(String(raw));
    const ok = ALLOWED_ORIGINS.some(a => {
      try { return new URL(a).origin === u.origin; } catch { return false; }
    });
    return ok ? u.origin : (process.env.SITE_URL || "https://beafrique.netlify.app");
  } catch {
    return process.env.SITE_URL || "https://beafrique.netlify.app";
  }
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // build line_items (your existing logic)...
    // const line_items = …

    const orderId = body.orderId || `ord_${Date.now()}`;
    const origin = pickReturnUrl(body.returnUrl);
