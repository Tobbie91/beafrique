...
export default function Checkout() {
  ...
  // Shipping form (optional – included in WhatsApp message)
  ...
  if (!items.length) {
    return (
      <div className="container py-12">
        ...
        <Link
          to="/products"
          className="mt-4 inline-block px-4 py-2 rounded bg-brand text-white"
        >
          Shop products
        </Link>
      </div>
    ...
    <div className="container py-10 grid lg:grid-cols-3 gap-8">
      <div className="lg:col-span-2">
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  value={i.qty}
                  onChange={(e) =>
                    updateQty(i.id, Math.max(1, Number(e.target.value)))
                  }
        ...
        <div className="mt-8 p-6 rounded-xl border">
          ...
          <p className="text-sm text-gray-600 mt-2">
            UK orders: 2–5 business days • International: 7–10 business days.
          </p>
        </div>
      </div>

      {/* Payment (Bank Transfer + WhatsApp) */}
      <aside className="h-fit p-6 rounded-xl border bg-white shadow-sm">
        ...
        {/* WhatsApp handoff */}
          <a
            href={
              `https://wa.me/${BRAND.whatsapp}?text=` + encodeURIComponent(waText)
            }
            target="_blank"
            rel="noreferrer"
          ...
          <button
            onClick={clear}
            className="mt-3 w-full rounded-lg border px-5 py-3 hover:bg-gray-50"
          >
            Clear Cart
          </button>
        </div>
        ...
        <p className="mt-4 text-xs text-gray-500">
          Your items are reserved for 24h while you complete transfer and send proof.
        </p>
      </aside>
    </div>
  );
