...
export async function startCheckout(payload: {
  items: {
    ...
    size?: string;
    color?: string;
    /** minor units, e.g. £65.00 -> 6500 */
    amount?: number;
    /** ISO currency, lowercase (e.g. "gbp") */
    currency?: string;
    /** OR a Stripe price id instead of amount/currency */
    priceId?: string;
  }[];
  orderId?: string;
  email?: string;
  ...
  const returnUrl = window.location.origin; // e.g. https://beafrique.com
