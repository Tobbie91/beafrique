...
export default function Header() {
  ...
  const link = (isActive: boolean) =>
    `px-3 py-2 rounded-md transition hover:bg-brand/10 hover:text-brand ${
      isActive ? "text-brand" : ""
    }`;
    ...
    <header className="sticky top-0 z-40 bg-white/80 backdrop-blur border-b">
      ...
      <nav className="container h-20 flex items-center justify-between mt-3">
        ...
        <div className="hidden md:flex items-center gap-2">
            ...
            {aboutOpen && (
              <div className="absolute left-0 mt-2 w-48 rounded-xl border bg-white shadow-soft p-2">
                <NavLink
                  to="/about#values"
                  className={({ isActive }) =>
                    `block px-3 py-2 rounded-lg hover:bg-brand/10 ${
          ...
          <NavLink to="/client" className={({ isActive }) => link(isActive)}>
            The Journey
          </NavLink>
          <NavLink to="/catalogue" className={({ isActive }) => link(isActive)}>
            Catalogue
          </NavLink>
          <NavLink
            to="/former-designs"
        ...
        <div className="md:hidden border-t bg-white">
          <div className="container py-3 flex flex-col">
            ...
            <details className="px-1 py-2">
              <summary className="cursor-pointer px-2 py-1 rounded-md hover:bg-brand/10">
                About Us
              </summary>
